...
description = pd.read_csv("C:\\Users\\sssnj\\OneDrive\\Desktop\\medical recomedation system\\description.csv")
precautions = pd.read_csv("C:\\Users\\sssnj\\OneDrive\\Desktop\\medical recomedation system\\precautions_df.csv")
workout = pd.read_csv("C:\\Users\\sssnj\\OneDrive\\Desktop\\medical recomedation system\\workout_df.csv")
medications = pd.read_csv('C:\\Users\\sssnj\\OneDrive\\Desktop\\medical recomedation system\\medications.csv')
diets = pd.read_csv("C:\\Users\\sssnj\\OneDrive\\Desktop\\medical recomedation system\\diets.csv")
...
def get_disease_details(disease):
    try:
        ...
        precautions_list = precautions[precautions['Disease'] == disease][['Precaution_1', 'Precaution_2', 'Precaution_3', 'Precaution_4']].values.tolist()[0]
        medications_list = medications[medications['Disease'] == disease]['Medication'].values.tolist()  # Ensure 'Medication' is the correct column name
        workout_text = workout[workout['disease'] == disease]['workout'].values[0]
        
        return description_text, precautions_list, medications_list, workout_text
...
@app.get("/")
def read_root():
    return {"Hello": "Welcome to the Disease Prediction API!"}

@app.post("/predict_disease", response_model=DiseaseResponse)
def predict_disease(symptoms: Symptoms):
    try:
        symptoms_indices = [symptoms_dict[symptom] for symptom in symptoms.symptoms]
        symptoms_array = np.zeros(len(symptoms_dict))
        symptoms_array[symptoms_indices] = 1
        symptoms_array = symptoms_array.reshape(1, -1)

        # Predict using the SVC model
        prediction = svc_model.predict(symptoms_array)[0]
        disease_name = diseases_list[prediction]

        # Fetch disease details
        description_text, precautions_list, medications_list, workout_text = get_disease_details(disease_name)
